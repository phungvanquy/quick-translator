...
| File | Purpose |
|---|---|
| `theme.css` | Indigo brand design tokens (dark + light via `prefers-color-scheme`), `.ic` icon helper, vector spinner keyframes |
| `icons.js` | Shared inline SVG `<symbol>` sprite (close, arrow-right, chat, spinner, volume, copy, check, refresh); injected as first `<body>` child |
| `popup.html/css/js` | Frameless translate popup: draggable header, vector spinner, streaming text, Esc+blur close |
...
- UI theme (change `redesign-ui-indigo-theme`): indigo/violet brand accent (`#7C6BFF` dark / `#6D5AE6` light), dark **and** light themes driven purely by `@media (prefers-color-scheme)` in `theme.css` — one token set, no in-app toggle, no JS wiring. Every component reads `var(--…)`; no literal hex outside `theme.css` except `#ffffff` on colored buttons. All UI emoji were replaced by a shared inline-SVG `<symbol>` sprite (`icons.js`) referenced via `<svg class="ic"><use href="#ic-…"/></svg>`; icons use `currentColor` so they re-tint per theme/state for free. Icon paths adapted from MIT Lucide.
- Popups use rounded corners + shadow via `.transparent(true)` on the translate/chat `WebviewWindowBuilder`s (Settings stays decorated). The rounded surface + `box-shadow` live in CSS on `body`/`.chat-window` over the transparent window. **OQ1 (unverified):** confirm on a real Windows build that transparency keeps shadow + always-on-top + positioning; if it misbehaves, revert to an opaque window and rely on CSS `border-radius` alone (square corners, acceptable) — record the outcome here.
- App icon (`icons/icon.png` 512px + `icons/icon.ico` multi-size) is a two-arrows ⇄ glyph on an indigo gradient, generated by the committed `icons/generate_icon.py` (Pillow, run once; outputs committed). The tray uses `default_window_icon()`, so it picks up the bundle icon automatically.
...
#### UI theme (change: redesign-ui-indigo-theme)

- [ ] Both popups + Settings render correctly in **dark** and **light** (toggle the OS color-scheme preference); text stays legible, indigo accent used for primary/link/focus, no washed-out or invisible elements
- [ ] All glyphs are SVG icons (close, language arrow, chat, spinner, copy→check, context-clear, reset) — no emoji anywhere; icons tint to match hover/disabled/theme; the spinner spins
- [ ] Popups show rounded corners + a drop shadow and are visually layered (header vs. body vs. bubbles); chat user bubble uses the indigo accent-soft tint, assistant bubble stays neutral. **OQ1:** confirm `transparent:true` didn't break shadow / always-on-top / cursor positioning on Windows — if it did, take the opaque + CSS-radius fallback (see design decision) and note it
- [ ] Tray icon and installer show the new two-arrows indigo brand icon and it's crisp at small sizes
...
- **Dependencies — minimal, not zero.** The original goal of few libs/deps is a preference for a lean binary, NOT a ban on external crates/libraries. It is fine to add and use a dependency when it makes the project meaningfully better, as long as it is safe: actively maintained, widely used, from a trusted source, pinned to an exact/known version, and checked against typosquatting. Prefer the lightest option that does the job, but don't reinvent something a vetted crate does well.
